...
** Remote agents with TRAMP

Running an agent on a remote host or container with TRAMP just works if the agent is installed on the host. For SSH, just open a remote file or directory =/ssh:host:/some/path= and then =M-x agent-shell=.
